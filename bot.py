...
def get_all_album_refs():
    refs = []
    # Albums are folders which contain actual songs/info/poster directly.
    roots = []
    ...
    if yw:
        ...
        roots = [{"id": x["id"]} for x in roots]
    for year in roots:
        for f in walk_folders(year["id"]):
            if has_album_content(f["id"]):
                refs.append({"id": f["id"], "name": get_folder_name(f["id"])})
    # Also support an album folder directly under root if needed.
    refs.sort(key=lambda x: x["name"].lower())
    seen = set()
    return [r for r in refs if not (r["id"] in seen or seen.add(r["id"]))]


def collect_songs(folder_id):
    result = []
    for x in get_items(folder_id):
        if is_song(x): result.append(x)
        elif is_folder(x): result.extend(collect_songs(x["id"]))
    return result
...
async def show_album_or_folder(chat, folder_id, old_message=None):
    ...
    # If there are subfolders and no direct songs, this is a navigation folder, NOT an album.
    if data["folders"] and not data["songs"]:
        kb=[[InlineKeyboardButton("📁 "+f["name"],callback_data=f"folder:{f['id']}")] for f in data["folders"]]
        kb.append([InlineKeyboardButton("⬅️ Back",callback_data=f"back:{folder_id}")])
        kb.append([InlineKeyboardButton("🏠 Main Menu",callback_data="home")])
        await chat.send_message("📁 <b>"+html.escape(data["name"])+"</b>\n\nSelect a folder:",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb)); return
...
async def show_folder(chat, folder_id, old_message=None):
    ...
    for f in data["folders"]:
        kb.append([InlineKeyboardButton("📁 "+f["name"],callback_data=f"folder:{f['id']}")])
    for s in data["songs"]:
        kb.append([InlineKeyboardButton("🎵 "+s["name"]+" ⬇️",callback_data=f"file:{s['id']}")])
    ...
    if parent:
        p_name=get_folder_name(parent).lower()
        if find_year_by_id(parent): back=f"year:{parent}"
        elif p_name in {"year wise","yearwise","years","year"}: back="cat:years"
        elif p_name in {"lofi","lofi song","lofi songs","lofi music"}: back="cat:lofi"
        else: back=f"folder:{parent}"
    else: back="home"
    ...
    if data["songs"] or data["images"] or data["info"]:
        ...
        if poster:
            ...
            try:
                ...
                with open(path,"rb") as f: await chat.send_photo(f,caption=caption,parse_mode="HTML")
            except Exception as e: print("POSTER ERROR",repr(e)); await chat.send_message(caption,parse_mode="HTML")
            finally:
                if path and os.path.exists(path): os.remove(path)
        else: await chat.send_message(caption,parse_mode="HTML")
    await chat.send_message("🎵 <b>"+html.escape(data["name"])+"</b>\n\nSelect a song:",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb))
...
async def show_lofi(chat, old_message=None):
    ...
    if not root:
        await chat.send_message("🎧 <b>Lofi Songs</b>\n\n❌ Lofi Songs folder കണ്ടെത്തിയില്ല.",parse_mode="HTML",reply_markup=category_keyboard()); return
    songs=collect_songs(root["id"]); songs.sort(key=lambda x:x.get("name","").lower())
    kb=[[InlineKeyboardButton("🎧 "+s["name"]+" ⬇️",callback_data=f"file:{s['id']}")] for s in songs[:100]]
    kb.append([InlineKeyboardButton("🏠 Main Menu",callback_data="home")])
    await chat.send_message("🎧 <b>Lofi Songs</b>\n\n🎵 Select a song:",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb))
...
async def show_latest(chat,page=0,old_message=None):
    songs=[]
    for y in get_year_folders(): songs.extend(collect_songs(y["id"]))
    lofi=find_lofi_folder()
    if lofi: songs.extend(collect_songs(lofi["id"]))
    seen=set(); songs=[s for s in songs if not (s["id"] in seen or seen.add(s["id"]))]
    songs.sort(key=lambda x:x.get("modifiedTime","") or "",reverse=True)
    total=max(1,(len(songs)+SONGS_PER_PAGE-1)//SONGS_PER_PAGE); page=max(0,min(page,total-1))
    if old_message:
        try: await old_message.delete()
        except Exception: pass
    kb=[[InlineKeyboardButton("🆕 "+s["name"]+" ⬇️",callback_data=f"file:{s['id']}")] for s in songs[page*SONGS_PER_PAGE:(page+1)*SONGS_PER_PAGE]]
    nav=[]
    if page: nav.append(InlineKeyboardButton("⬅️",callback_data=f"latestpage:{page-1}"))
    nav.append(InlineKeyboardButton(f"📄 {page+1} / {total}",callback_data="nothing"))
    if page<total-1: nav.append(InlineKeyboardButton("➡️",callback_data=f"latestpage:{page+1}"))
    if nav: kb.append(nav)
    kb.append([InlineKeyboardButton("🏠 Main Menu",callback_data="home")])
    await chat.send_message("🆕 <b>Latest Songs</b>\n\nപുതിയായി add ചെയ്ത songs:",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb))


def search_drive(folder_id,text):
    out=[]; q=text.lower()
    for x in get_items(folder_id):
        if q in x.get("name","").lower(): out.append(x)
        if is_folder(x): out.extend(search_drive(x["id"],text))
    return out


async def perform_search(message, search_text):
    results=search_drive(ROOT_FOLDER_ID,search_text)
    songs=[x for x in results if is_song(x)]
    folders=[x for x in results if is_folder(x)]
    kb=[]
    for s in songs[:50]: kb.append([InlineKeyboardButton("🎵 "+s["name"]+" ⬇️",callback_data=f"file:{s['id']}")])
    for f in folders[:30]: kb.append([InlineKeyboardButton("💿 "+f["name"],callback_data=f"album:{f['id']}")])
    kb.append([InlineKeyboardButton("🏠 Main Menu",callback_data="home")])
    await message.reply_text(f"🔍 <b>Search:</b> {html.escape(search_text)}\n\nFound: {len(songs)} song(s)",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(kb))
...
async def send_request_to_admin(update,context,text):
    ...
    msg=await context.bot.send_message(ADMIN_CHAT_ID,f"📩 <b>New Song Request</b>\n\n🆔 User ID: {user.id}\n👤 {html.escape(user.full_name or '')}\n\n🎵 {html.escape(text)}",parse_mode="HTML")
