...
def format_published_date(value):
    """Format Google Drive createdTime as DD Month YYYY in India time."""
    if not value:
        return "N/A"
    try:
...
def tmdb_request(path, params):
    """Get data from TMDB. Returns None when no API key is configured."""
    if not TMDB_API_KEY:
        return None
    try:
        params = dict(params)
        ...
        url = "https://api.themoviedb.org/3/" + path.lstrip("/") + "?" + urllib.parse.urlencode(params)
        req = urllib.request.Request(url, headers={"User-Agent": "A2Z-Malayalam-Songs-Bot/1.0"})
...
def get_year_from_folder_name(name):
    m = re.search(r"\\b(19|20)\\d{2}\\b", name or "")
    return m.group(0) if m else ""


def clean_album_name(name):
    return re.sub(r"\\s*[\\(\\[]?(?:19|20)\\d{2}[\\)\\]]?\\s*$", "", name or "").strip()


def automatic_movie_metadata(folder_name):
    """
    Fill missing movie metadata automatically from the Drive folder name.
    TMDB provides director, principal cast and composer credits.
    Singer is left empty unless Album-Info.txt supplies it, because movie
    databases do not reliably expose playback singers for every film.
    """
    year = get_year_from_folder_name(folder_name)
    title = clean_album_name(folder_name)
    cache_key = (title.lower(), year)

    if cache_key in _metadata_cache:
        return dict(_metadata_cache[cache_key])

    result = {"year": year, "director": "", "singer": "", "music": "", "artists": ""}
    ...
    if data and data.get("results"):
        ...
        if movie.get("release_date"):
            result["year"] = movie["release_date"][:4]
        ...
        if movie_id:
            ...
            if credits:
                ...
                artists = [
                    c.get("name") for c in cast[:10]
                    if c.get("name")
                ]

                result["director"] = ", ".join(dict.fromkeys(directors))
                result["music"] = ", ".join(dict.fromkeys(composers))
                result["artists"] = ", ".join(dict.fromkeys(artists))

    _metadata_cache[cache_key] = dict(result)
    return result


def get_album_data(folder_id):
    folders, songs, images, info_file, other = get_folder_contents(folder_id)
    folder_meta = get_file(folder_id)
    name = folder_meta.get("name", "Album")

    # Album-Info.txt remains supported. Its values override automatic lookup.
    info = parse_info(read_info_file(info_file["id"]) if info_file else "", name)
    auto = automatic_movie_metadata(name)

    for key in ("year", "director", "singer", "music", "artists"):
        if not info[key]:
            info[key] = auto[key]

    # Count songs recursively, including songs inside subfolders.
    all_songs = collect_songs(folder_id)
...
def album_caption(album):
    ...
    t += f"\n🎶 <b>Total Tracks</b> : {len(album.get("all_songs", album["songs"]))}"
    t += "\n📅 <b>Published On</b> : " + html.escape(format_published_date(album.get("createdTime")))
    return t


def album_card_caption(album):
    i = album["info"]
    return f"💿 <b>{html.escape(i['album'])}</b>\n📅 {html.escape(i['year'] or 'N/A')}\n🎶 Tracks : {len(album.get("all_songs", album["songs"]))}" + (f"\n🏷️ Label : {html.escape(i['label'])}" if i["label"] else "")
...
async def show_folder(chat, folder_id, old_message=None):
    ...
    display_songs = data.get("all_songs", data["songs"]) if not data["folders"] else data["songs"]
    kb = [[InlineKeyboardButton("🎵 " + s["name"] + " ⬇️", callback_data=f"file:{s['id']}")] for s in display_songs]
